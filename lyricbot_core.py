...
def _history_file(config):
    return os.path.join(config['ASSET_FOLDER'], 'run_history.json')
...
def run_workflow(config):
    ...
    # --- NEW: Verify Google Drive access at the start, but do not create folders yet. ---
    drive_service = None
    ...
    for attempt in range(1, MAX_QUALITY_CHECK_ATTEMPTS + 1):
        ...
        for i, (song_info, pre_fetched_lrcs, pre_fetched_audios) in enumerate(songs_to_process):
            # --- Initialize Google Drive variables here, inside the loop ---
            # --- FIX: Initialize run_folder_id to None at the start of each song loop ---
